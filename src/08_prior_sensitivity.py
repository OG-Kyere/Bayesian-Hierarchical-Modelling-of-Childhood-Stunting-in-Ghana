...
Refits the complete-case Model 3 under tighter and wider weakly-regularizing
priors. Primary specification: beta SD=1, random-effect SD prior scale=1.
"""
...
def fit(label, beta_sd, tau_sd, seed):
    ...
    with pm.Model(coords=coords):
        ...
        beta = pm.Normal("beta", 0, beta_sd, dims="coef")
        tau_c = pm.HalfNormal("tau_community", tau_sd)
        z_c = pm.Normal("z_community", 0, 1, dims="community")
        tau_h = pm.HalfNormal("tau_household", tau_sd)
    ...
    idata.to_netcdf(OUT / f"model3_prior_{label}.nc")

def main():
    fit("tight", beta_sd=0.5, tau_sd=0.5, seed=20262100)
    fit("wide", beta_sd=1.5, tau_sd=1.5, seed=20262200)
