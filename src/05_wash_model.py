"""Model 2: household/community Bayesian model extended with WASH covariates.

Two fits are supported:
1. full analytic sample (4,928 children), and
2. harmonized complete-maternal-information sample used for comparison with Model 3.
"""
...
def main():
    d, parts = prepare_base(complete_maternal=False)
    X = pd.concat(parts, axis=1).astype(float)
    fit_hierarchical(d, X, "model2_wash_full", seed=20261122)

    d_h, parts_h = prepare_base(complete_maternal=True)
    X_h = pd.concat(parts_h, axis=1).astype(float)
    fit_hierarchical(d_h, X_h, "model2_wash_harmonized", seed=20261222)
