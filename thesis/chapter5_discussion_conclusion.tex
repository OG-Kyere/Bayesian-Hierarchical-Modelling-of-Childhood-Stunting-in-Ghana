...
Fifth, the analysis includes several robustness checks: alternative prior specifications, survey-weight sensitivity, explicit treatment of missing maternal-education information, an independent GEE clustering diagnostic, posterior predictive checks, and predictive model comparison.
...
All principal Model 1--3 fits had zero divergent transitions and fixed-effect parameters showed good chain agreement. However, the household and community standard deviations mixed more slowly, particularly in the harmonized WASH and maternal-education models. The current estimates suggest a household/community contrast, but do not yet establish it with adequate computational validation. Both the contrast and its uncertainty require confirmation from validated production runs.

\subsection{Model Comparison}

The WAIC comparison was conditional on fitted random effects and involved a high-dimensional hierarchical model. The difference between Model 2 and Model 3 was very small relative to its uncertainty, and some observations showed substantial pointwise log-likelihood variability. The comparison is therefore interpreted as evidence of similar predictive adequacy rather than as a definitive ranking of the models.
...
A spatial extension using authorized DHS GPS data could separate spatially structured community variation from non-spatial cluster heterogeneity. Such a model would complement the household/community decomposition developed in this report.
...
Finally, the random-effect variance parameters should be re-estimated with longer MCMC chains in the final production environment, and the posterior predictive checks should be extended to household- and community-level counts before journal submission.
