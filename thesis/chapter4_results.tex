...
\begin{quote}\textbf{Exploratory results---not validated publication findings.} The numerical Bayesian results in this chapter come from legacy short runs. The repaired pipeline has not yet established that these results meet the final diagnostic and sensitivity requirements. They must not be cited as confirmed findings. Verified survey descriptive results are distinguished from these provisional model estimates.\end{quote}
...
Across all of these fits, no divergent NUTS transitions were observed. Fixed-effect coefficients showed good cross-chain agreement, with $\hat R$ values close to 1.00. The hierarchical standard-deviation parameters mixed more slowly than the fixed effects; maximum $\hat R$ values were approximately 1.018 for Model 1, 1.012 for Model 2 on the full sample, 1.036 for the harmonized Model 2, and 1.033 for Model 3. Consequently, all inferential findings remain provisional. Apparently satisfactory fixed-effect diagnostics do not establish adequate exploration of the joint posterior when hierarchical variance parameters have not converged.
...
The fixed-effect parameters in Models 1--3 showed satisfactory chain agreement and no divergent transitions were observed. The hierarchical variance parameters were more computationally demanding, particularly in the harmonized Model 2 and Model 3, where $\hat R$ for the community SD was approximately 1.036 and 1.033, respectively. These diagnostics prevent treating the household/community contrast as confirmed. Consistency across short runs is encouraging but does not substitute for convergence of the joint posterior.
...
The predictive difference is therefore negligible relative to its uncertainty and does not support a claim that adding maternal education materially improves predictive accuracy. At the same time, the WAIC diagnostics indicated substantial pointwise log-likelihood variability for a number of observations, which is common in high-dimensional hierarchical models and reduces confidence in fine distinctions between the models. This comparison is inconclusive. The warnings and short runs prevent a reliable predictive ranking or a claim of equivalence. Conditional WAIC does not assess prediction in new households or communities.
...
The current results are exploratory and do not constitute the final publication analysis. Before journal submission, the hierarchical variance parameters will be confirmed with longer production runs in a stable computational environment. The existing posterior predictive checks and predictive model comparison will then be regenerated from the final locked posterior objects to ensure that the reported publication tables and figures correspond exactly to the production fits.
