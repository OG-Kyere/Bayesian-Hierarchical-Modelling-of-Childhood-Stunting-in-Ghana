...
After maternal education was added in Model 3, the posterior association for unimproved drinking water remained essentially unchanged (OR 1.50, 1.15--1.99), with $P(\text{OR}>1\mid\text{data})\approx0.998$. The sanitation estimate remained comparatively uncertain (OR 1.12, 0.86--1.46; $P(\text{OR}>1\mid\text{data})\approx0.79$).
...
The earlier WAIC comparison led to the same substantive conclusion but showed greater pointwise instability. PSIS-LOO is therefore used as the preferred predictive comparison. The result does not diminish the substantive interpretation of the maternal-education coefficient: Model 3 adds an interpretable association, particularly for higher maternal education, without evidence of a meaningful improvement in predictive performance over the harmonized WASH model.

\subsection{Remaining Production Checks}

The extended Model 3 run substantially strengthened the fixed-effect and sampler diagnostics and the PSIS-LOO comparison is now based on pointwise posterior log-likelihood draws. The remaining computational caution concerns the slower convergence of the household and community standard deviations, whose $\hat R$ values remain modestly above the conservative 1.01 publication target despite effective sample sizes above 500 in the extended run. A final unrestricted production run with longer within-chain trajectories is therefore desirable for locking the exact variance-component intervals, but the principal fixed-effect conclusions are no longer dependent on the shorter preliminary chains.
