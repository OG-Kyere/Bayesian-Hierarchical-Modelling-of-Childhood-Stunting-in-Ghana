...
This chapter presents the empirical findings from the 2022 Ghana Demographic and Health Survey. It begins with the analytic sample and survey-weighted descriptive patterns, then reports the sequential Bayesian hierarchical models. The chapter also covers prior, survey-weight, missing-data, age-functional-form, and detailed-WASH sensitivity analyses, posterior predictive checks, independent GEE checks, and the PSIS-LOO comparison of the harmonized WASH and maternal-education models. The eight-chain Model 3 results reported here are the locked primary results for the current manuscript.
...
\caption[Stunting prevalence by drinking-water source]{Survey-weighted stunting prevalence by improved versus unimproved drinking-water source type.}
...
Across the initial sequential fits, no divergent NUTS transitions were observed and fixed-effect coefficients showed good cross-chain agreement. The hierarchical standard-deviation parameters mixed more slowly, which motivated the strengthened eight-chain Model 3 diagnostic run reported below. The locked Model 3 estimates come from that extended run rather than from the shorter preliminary fit.
...
After maternal education was added in Model 3, the posterior association for unimproved drinking water remained essentially unchanged (OR 1.50, 1.15--1.99), with $P(\text{OR}>1\mid\text{data})\approx0.998$. The sanitation estimate remained comparatively uncertain (OR 1.12, 0.86--1.46; $P(\text{OR}>1\mid\text{data})\approx0.79$). In the locked eight-chain run, boys had higher odds than girls (OR 1.50, 1.24--1.82), children aged 24--35 months had higher odds than those aged 0--5 months (OR 2.91, 2.06--4.21), and the richest-versus-poorest OR was 0.36 (0.20--0.64).

Maternal higher education showed the clearest education association. Relative to children whose mothers had no formal education, children whose mothers had higher education had substantially lower posterior odds of stunting (OR 0.37, 0.20--0.65), with $P(\text{OR}<1\mid\text{data})\approx0.999$. Primary education was not clearly separated from no education (OR 1.02, 0.75--1.41), and the secondary-education estimate was also compatible with a range of modest protective or null associations (OR 0.89, 0.68--1.14).

The wealth gradient attenuated after maternal education entered the model. For example, the richest-versus-poorest OR changed from 0.26 in the harmonized WASH model to 0.36 in Model 3, although the posterior interval remained clearly below one (0.20--0.64). This attenuation is consistent with overlap between maternal education and household socioeconomic position, but the cross-sectional design does not permit a causal mediation interpretation.
...
Model 3: Male vs female & 1.50 & 1.24--1.82 \\
Model 3: Age 24--35 vs 0--5 months & 2.91 & 2.06--4.21 \\
Model 3: Richest vs poorest & 0.36 & 0.20--0.64 \\
Model 3: Unimproved water & 1.50 & 1.15--1.99 \\
Model 3: Unimproved/no sanitation & 1.12 & 0.86--1.46 \\
Model 3: Maternal primary vs none & 1.02 & 0.75--1.41 \\
Model 3: Maternal secondary vs none & 0.89 & 0.68--1.14 \\
Model 3: Maternal higher vs none & 0.37 & 0.20--0.65 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Model 3 & 0.389 & 1.234 & 0.030 & 0.307 \\
\bottomrule
\end{tabular}%
}
...
The fixed-effect parameters in Models 1--3 showed satisfactory chain agreement and no divergent transitions were observed. To strengthen the final-model diagnostics, Model 3 was subsequently refitted using eight independent chains with 500 warmup and 500 retained draws per chain, yielding 4,000 retained posterior draws. Across this extended run there were no divergent transitions, chain-specific BFMI values ranged from 0.51 to 0.66, and no chain reached the maximum tree depth. The fixed effects showed excellent convergence, with a maximum $\hat R$ of approximately 1.003 and all bulk effective sample sizes exceeding 2,200. The hierarchical standard deviations remained the slowest-mixing quantities: $\hat R$ was approximately 1.021 for the community SD and 1.018 for the household SD, with bulk effective sample sizes of approximately 544 and 524, respectively. Thus, the substantive fixed-effect conclusions are strongly supported, while the precise variance-component intervals remain somewhat more Monte Carlo sensitive.

\subsection{Prior Sensitivity}

Model 3 was refitted under tighter and wider prior scales for the regression coefficients and hierarchical standard deviations. The principal fixed-effect conclusions were stable. Under the tighter prior, the OR for unimproved drinking water was 1.48 (95\% posterior interval: 1.14--1.88), while under the wider prior it was 1.48 (1.12--2.06), compared with 1.50 (1.15--1.99) in the locked primary run. The maternal higher-education OR was 0.44 (0.26--0.70) under the tighter prior and 0.35 (0.18--0.66) under the wider prior, compared with 0.37 (0.20--0.65) in the locked primary run.
...
The strengthened weighted sensitivity fit completed without divergences, with a maximum $\hat R$ of about 1.01 and minimum bulk effective sample size above 400. Even with acceptable sampling, its variance components are not used as substitutes for the primary decomposition because the survey-weight pseudo-posterior changes the target and the interpretation of hierarchical variance. The weighted fit is therefore used chiefly to assess whether the fixed-effect conclusions are sensitive to the treatment of survey weights.
...
\subsection{Age Functional-Form Sensitivity}

The primary models represent child age using six interpretable age groups. To assess whether the substantive conclusions depended on those cut points, Model 3 was refitted with child age represented by a four-degree-of-freedom cubic B-spline while retaining the same household and community random effects and the remaining covariates.

The major posterior associations were essentially unchanged. Under the spline-age specification, the OR for unimproved drinking water was 1.49 (95\% posterior interval: 1.12--2.00), the sanitation OR was 1.11 (0.86--1.44), the richest-versus-poorest OR was 0.35 (0.19--0.61), the male-versus-female OR was 1.53 (1.27--1.86), and the maternal higher-education OR was 0.37 (0.20--0.67). Fixed-effect convergence was strong, with a maximum $\hat R$ of approximately 1.005 and all bulk effective sample sizes above 900. These results indicate that the principal substantive conclusions are not artifacts of categorizing child age into six bands.

\subsection{Alternative WASH Coding Sensitivity}

The binary improved/unimproved WASH coding was also relaxed to distinguish improved water, unprotected groundwater, and surface water, and to distinguish improved sanitation, other unimproved sanitation, and open defecation.

The clearest water association was observed for surface-water use. Relative to improved sources, the posterior median OR for surface water was 1.52 (95\% posterior interval: 1.10--2.08). The corresponding estimate for unprotected groundwater was 1.41 (0.90--2.21), indicating greater uncertainty. For sanitation, open defecation had an OR of 1.23 (0.92--1.64), while other unimproved sanitation had an OR of 0.93 (0.68--1.31). Higher maternal education remained strongly inversely associated with stunting (OR 0.38, 0.20--0.67). Fixed-effect diagnostics were again strong. This sensitivity analysis suggests that the primary binary water association is driven most clearly by surface-water exposure, whereas the adjusted sanitation findings remain comparatively weak.
...
The harmonized Model 2 and Model 3 were compared on the same 4,503 children using Pareto-smoothed importance-sampling leave-one-out cross-validation (PSIS-LOO). Model 2 had an expected log predictive density (ELPD) of approximately -2012.39, while the extended Model 3 run had an ELPD of approximately -2011.47. The Model 3 minus Model 2 difference was therefore only 0.92 ELPD units, with an estimated standard error of 3.38.

The predictive difference is negligible relative to its uncertainty and does not support a claim that adding maternal education materially improves out-of-sample predictive accuracy. Model 3 nevertheless had cleaner importance-sampling diagnostics: its maximum Pareto $k$ was approximately 0.69, with no observations exceeding 0.70, whereas Model 2 had a maximum Pareto $k$ of approximately 0.79 and five observations above 0.70; neither model had an observation with Pareto $k>1$.

An earlier WAIC calculation led to the same broad conclusion, but the final project uses PSIS-LOO because it provides direct pointwise influence diagnostics through Pareto $k$. The result does not diminish the substantive interpretation of the maternal-education coefficient: Model 3 adds an interpretable association, particularly for higher maternal education, without evidence of a meaningful improvement in predictive performance over the harmonized WASH model.

\subsection{Final Diagnostic Status}

The eight-chain Model 3 run substantially strengthened the fixed-effect and sampler diagnostics, and the PSIS-LOO comparison is based on pointwise posterior log-likelihood draws. The household and community standard deviations remain the slowest-mixing parameters, with $\hat R$ values modestly above 1.01 despite bulk effective sample sizes above 500. For that reason, their interval endpoints deserve more Monte Carlo caution than the fixed effects. The household-versus-community contrast itself is stable across the completed checks and is treated as locked for the current manuscript.
...
After adjustment for demographic, socioeconomic, regional, household, and community structure, unimproved drinking-water source remained associated with higher posterior odds of stunting. Sanitation showed a weaker and less certain adjusted association. Maternal higher education was associated with substantially lower posterior odds of stunting, while primary and secondary education were not clearly separated from no formal education after full adjustment. The household variance remained substantial after WASH and maternal education were introduced, indicating that important unmeasured household-level influences remain. Prior, survey-weight, missing-maternal-information, age-functional-form, and WASH-coding sensitivity analyses broadly supported the main fixed-effect conclusions, although the water association was less precise under the weighted pseudo-posterior. PSIS-LOO indicated essentially equivalent predictive performance for the harmonized WASH and maternal-education models. The variance-component intervals warrant more caution than the fixed-effect intervals because those parameters mixed more slowly, but the locked eight-chain run and the independent local reproduction support the same household-versus-community pattern.
