...
The target population for the present study is children aged 0--59 months residing in sampled Ghanaian households at the time of the 2022 GDHS.
...
\begin{enumerate}
    \item were usual household members, operationalized by \texttt{hv103 = 1};
...
Applying these rules produced a preliminary analytic sample of 4,928 children distributed across 616 communities and 3,545 households. These figures refer to the analytic sample constructed from the PR file and should not be confused with the total number of children measured in the survey report. The analytic sample will be rechecked after all required household, WASH, and maternal variables have been merged and the final complete-case or harmonized modelling sample has been defined.
...
Water and sanitation variables will be introduced in the extended model after harmonization of the Household and Household Member recodes. Household drinking-water source and sanitation facility will be classified using DHS categories and, where appropriate, recoded into substantively interpretable groups such as improved versus unimproved services in accordance with established WHO/UNICEF definitions.
...
Maternal education will be introduced in a subsequent model. Where the biological mother can be reliably linked to the child within the household or recode structure, education will be represented using DHS educational attainment categories. The final coding and treatment of children whose mothers are deceased, absent, or not successfully linked will be documented explicitly.

The maternal-education extension will be compared with preceding models on a harmonized sample so that changes in coefficients or random-effect variances are not mistakenly attributed to changes in sample composition.
...
Weighted estimates will be used for quantities such as national stunting prevalence and weighted subgroup prevalence. Unweighted counts will be reported where they describe the actual analytic sample size.

The handling of survey weights in Bayesian hierarchical inference requires additional care because weighting, clustering, and hierarchical random effects address different aspects of the data-generating and sampling processes. Therefore, the study will not treat the survey weight as a mechanical replacement for multilevel structure. The primary hierarchical analysis and alternative weighted or pseudo-posterior specifications will be compared as part of a planned sensitivity analysis.
...
The final manuscript may present a reduced subset of these models if some stages are primarily diagnostic or sensitivity analyses, but the thesis will document the full sequence.
...
Each standardized log-odds coefficient is assigned an independent prior
...
The household-level standard deviation in Model 1 and subsequent models will use the same family of weakly regularizing half-normal prior unless prior-predictive assessment indicates a need for revision.
...
Alternative prior scales will be evaluated in the sensitivity analysis. The objective is not to select priors that reproduce the observed results, but to ensure that the primary priors are scientifically defensible and not excessively restrictive or diffuse.

\section{Posterior Computation}

Posterior sampling is conducted in Python using PyMC. The preliminary model was fitted with the No-U-Turn Sampler (NUTS), an adaptive Hamiltonian Monte Carlo algorithm.
...
The preliminary model was fitted using PyMC version 6.3.2, with posterior summaries and diagnostics calculated using ArviZ version 1.3.0. Package versions will be recorded in the repository for the final analysis so that the computational environment can be reconstructed.
...
\begin{itemize}
    ...
    \item rank-normalized potential scale reduction statistics, (\hat{R});
...
while the household-plus-community correlation for two children in the same household is
...
Because the GDHS uses unequal sampling probabilities, a dedicated sensitivity analysis will examine whether the main posterior conclusions change when survey weights are incorporated using a defensible pseudo-likelihood or other appropriate weighting strategy.

At minimum, the sensitivity analysis will compare:

\begin{enumerate}
    \item the primary hierarchical specification;
    \item a corresponding specification incorporating rescaled survey weights; and
    \item weighted descriptive estimates used as external checks on model-implied population patterns.
\end{enumerate}

Any differences in coefficients, credible intervals, variance components, and posterior predictive quantities will be reported transparently. The final thesis will specify the exact rescaling and likelihood implementation once the weighted models have been fitted and validated.

\section{Prior Sensitivity Analysis}

Sensitivity to prior specification will be examined by refitting the principal model under alternative reasonable prior scales for fixed effects and hierarchical standard deviations. The analysis will focus on whether substantive conclusions, especially the household and community variance components and key odds ratios, are materially altered.

A conclusion will be considered robust when its posterior magnitude and uncertainty remain substantively similar under reasonable prior alternatives.
...
The amount and pattern of missingness for key variables will be described. If missingness is sufficiently extensive to threaten complete-case inference, alternative strategies such as multiple imputation or an explicitly Bayesian missing-data model will be considered before final analysis. Such a decision will be made based on the observed data pattern rather than assumed in advance.
