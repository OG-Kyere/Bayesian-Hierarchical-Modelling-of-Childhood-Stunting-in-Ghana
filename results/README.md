...
The current predictive comparison is based on **PSIS-LOO**. The locked manuscript comparison is stored in `tables/loo_model_compare_final.csv`. The file `tables/model_predictive_comparison_loo.csv` may be regenerated by a local reproducibility run and should not replace the locked comparison unless `src/09_model_comparison_ppc.py --write-final` is run intentionally. Older WAIC outputs remain part of the analysis history but are not the main model-comparison result used in the manuscript.
