...
We analysed the Household Member Recode from the 2022 GDHS under authorized access from The DHS Program. The GDHS used a stratified two-stage cluster sample \citep{GSSICF2024}. Children were eligible for this analysis if they were aged 0--59 months, had slept in the sampled household on the night before the interview (\texttt{hv103 = yes}), and had a valid height-for-age z-score in \texttt{hc70}.
...
We then challenged the main specification in several ways. We refitted Model 3 with tighter and wider priors; fitted a pseudo-posterior using survey weights rescaled to mean one; retained children with missing maternal education as a separate sensitivity category; replaced the age groups with a cubic B-spline; and replaced the binary WASH variables with more detailed water and sanitation categories. Predictive performance of harmonized Models 2 and 3 was compared with PSIS-LOO and Pareto-$k$ diagnostics.

\subsection{Ethics and data access}

The analysis used de-identified secondary data obtained through authorized DHS access. The original GDHS obtained informed consent under its survey protocol. We do not redistribute DHS microdata. The public reproducibility repository contains code and aggregate, non-identifying outputs only.
